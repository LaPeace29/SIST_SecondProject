...
public class InstructorDAO {
	...
	// 강사 출력 메소드(1)
	// 강사이름 / 강사휴대폰번호
	public List<Instructor> print1(String instructor_id) {
		...
		try {
			conn = OracleConnection.connect();
			String sql = "SELECT instructor_id, instructor_name, instructor_phone\r\n" + 
					"    FROM instructor\r\n" + 
			...
			while(rs.next()) {
				...
				Instructor i = new Instructor(instructor_id1, instructor_name, instructor_phone, null);
				list.add(i);
			}
		} catch (ClassNotFoundException e) {
			e.printStackTrace();
